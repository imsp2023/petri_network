...
QUnit.test("the basic shape of a manual transition is a rectange with an arrow above", assert =>{
    ...
    assert.equal(t.shape.shape.height, MHeight, "the height must be 40 px");
...
QUnit.test("the basic shape of an automatic transition is a black rectange with its name above", assert =>{
    ...
    assert.equal(t.shape.shape.height, AHeight, "the height must be 35 px");
    assert.equal(t.shape.shape.fill, "white", "the color must be black");
    assert.equal(t.shape.shape["stroke-width"], TStroke, "the border width must  be 3 px");
        assert.equal(t.shape.shape.children[0].child.type, "text", "type of child is text");
    assert.equal(t.shape.shape.children[0].child.text, "foobar", "text value is tranisition name");
});
...
QUnit.test("the basic shape of a event transition is a rectange with a clock above", assert =>{
    ...
    assert.equal(t.shape.shape.children[0].child.type, "image", "type of child is image");
    assert.equal(t.shape.shape.children[0].child.path, "src/images/clock.png", "clock image path");
...
QUnit.test("addPanel() opens an empty panel when we hover over a transition", assert => {
    ...
    assert.ok(t.shape.shape.children.length, "the shape has a child");
    assert.equal(t.shape.shape.children[0].child.type, "rectangle", "panel has a rectangle as support");
...
QUnit.test("panel must cover the transition", assert => {
    ...
    assert.ok(t.shape.shape.children.length, "the shape has a child");
    assert.equal(t.shape.shape.children[0].child.type, "rectangle", "panel has a rectangle as support");
    assert.equal(t.shape.shape.children[0].child.offsetX, -2, "panel offsetX");
    assert.equal(t.shape.shape.children[0].child.offsetY, 0, "panel offsetY");
...
QUnit.test("add mouseleave event on panel", assert => {
    ...
    var ev = null;
    for (e of t.shape.shape.children[0].child.events)
	if (e["mouseleave"])
	    ev = e;
